...
class ImageGenerator:
    ...
    def _generate_from_photo(self, prompt: str, photo_part) -> bytes:
        ...
        config_kwargs = dict(
            ...
            # editar el contenido real que le mandamos. -1 = AUTOMATIC (el
            # modelo decide cuanto pensar) -- 0 seria DESHABILITADO, que es
            # lo que las llamadas de QC de texto usan a proposito.
    ...
    def _validate_product_photo_generation(self, image_bytes: bytes) -> bool:
        ...
        try:
            ...
            prompt = (
                ...
                "text_is_correct_spanish: if has_text is true, judge ONLY whether the "
                "legible words are real, valid Spanish words or brand names — not invented "
                "gibberish letter salad. This image often shows ORIGINAL text that already "
                "existed on the product itself (printed on packaging, balloons, wrappers, "
                "labels), which the edit may leave blurred, partially cropped, warped, or "
                "stylized — that visual imperfection alone is NOT an error. Only flag true "
                "nonsense words that are not real Spanish. If has_text is false, respond "
                "true (not applicable).\n"
                "is_abstract_3d: true if the image has floating 3D geometric shapes, abstract CGI objects, or surreal renders.\n"
                "has_screen_content: true if any computer monitor, laptop screen, phone screen, TV, or digital display "
                "shows visible content — including websites, text, images, graphics, UI elements, or any non-blank content. "
